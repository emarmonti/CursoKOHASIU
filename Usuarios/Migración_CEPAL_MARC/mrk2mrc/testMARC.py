...
class TestMARC(unittest.TestCase):
	def setUp(self):
		self.p = MARC.Parser()
		self.mp = MARC.MnemParser()

	def testParser(self):
		self.assertEqual(1, self.p.parse(record))
		self.assertEqual(0, self.p.eof())
		self.assertEqual(1, len(self.p.records))
		r = self.p.records[0]
		self.assertEqual(record, r.get())
		self.assertEqual(mnem, r.get_mnem())

	def testMnemParser(self):
		self.assertEqual(1, self.mp.parse(mnem))
		self.assertEqual(0, self.mp.eof())
		self.assertEqual(1, len(self.mp.records))
		r = self.mp.records[0]
		self.assertEqual(record, r.get())
		self.assertEqual(mnem, r.get_mnem())
